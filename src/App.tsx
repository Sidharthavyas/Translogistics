import {
  Phone, Menu, Truck, Globe, Factory, Search, CheckCircle,
  MapPin, ShieldCheck, CreditCard, Mail, MessageSquare, Home,
  ArrowRight, ArrowUpRight, ChevronRight, X, Clock, Package, Route,
} from 'lucide-react';
...
/* ═══════════════════════════════════════════════════════════
   ...
   Hoisted out of the component so they're created once,
   never re-allocated on re-renders, and share a single
   reference identity (important for React diffing).
   ═══════════════════════════════════════════════════════════ */
...
const ABOUT_FEATURES = [
  { icon: ShieldCheck, title: 'Hand‑Picked Operators', desc: 'Every driver personally vetted' },
  { icon: MapPin, title: 'Mumbai Hub', desc: 'Naigaon / Vasai base' },
  { icon: Phone, title: 'Direct Line', desc: 'You talk to us, not a machine' },
  { icon: CreditCard, title: 'Honest Pricing', desc: 'Transparent, no hidden costs' },
] as const;

const SERVICES_DATA = [
  { icon: Truck, title: 'Full Truck Loads', desc: 'Dedicated vehicles, direct routes, your cargo only.' },
  { icon: Globe, title: 'Pan‑India Routes', desc: 'Every state — metro hubs to remote sites.' },
  { icon: Factory, title: 'Industrial & Heavy Cargo', desc: 'Equipment, raw materials, oversized goods.' },
  { icon: Search, title: 'Vehicle Sourcing', desc: 'We find the right truck for your specific load.' },
  { icon: CheckCircle, title: 'We Handle Everything', desc: 'Paperwork, loading, tracking, delivery — all of it.' },
  { icon: Package, title: 'Custom Needs', desc: "Unusual cargo? Tell us. We'll figure it out." },
] as const;

const PROCESS_STEPS = [
  { step: '01', title: 'You Call or Message', desc: 'Tell us what, where, and when.', icon: MessageSquare },
  { step: '02', title: 'We Find the Right Truck', desc: 'From our personally vetted network.', icon: Search },
  { step: '03', title: 'We Coordinate Pickup', desc: 'And keep you updated throughout.', icon: Route },
  { step: '04', title: 'Delivered & Confirmed', desc: 'Safe delivery, paperwork sorted.', icon: CheckCircle },
] as const;
...
const HERO_IMAGE_URL =
  'https://lh3.googleusercontent.com/aida/ADBb0uhV2m87Gsn4lC4k50aQLiUfoGE4233LspZ7wLXa5k0wGF29MxDTmXYA5IA9DfYP7UbRpQUK6RbeF5FjGw9nmAsVnXaUnAYSpnPctin7kL8NCFVIK8dXZxkgIcLMQObZWlHsPrqzhtyXo1BHUqQaqCjH3jGoE23LuZHW5DnOQky3gx5k94AI2_WC7hGP__hrsIqvjoxRo0jzailQQWy5d-BFeAQo6aB61vTTOGte6OOCICpCjMOrZFSofJ06NlmvwdagI8NjqH7B';

/* ─── Animation presets (stable references) ─── */
...
/**
 * CountUp — optimized:
 * • `hasAnimated` is a ref (no re-render when set)
 * • Observer disconnects immediately after trigger
 * • rAF id tracked and cancelled on unmount (no memory leak)
 * • Uses ** operator instead of Math.pow (micro-opt)
 */
      ...
      ([entry]) => {
        if (entry.isIntersecting && !hasAnimated.current) {
          ...
          const step = (ts: number) => {
            ...
            setCount(Math.floor((1 - (1 - progress) ** 3) * target));
...
export default function App() {
  ...
  useEffect(() => {
    ...
    style.overflow = mobileMenuOpen ? 'hidden' : '';
    style.touchAction = mobileMenuOpen ? 'none' : '';
    return () => {
      style.overflow = '';
      style.touchAction = '';
    };
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      requestAnimationFrame(() => {
        ...
        // Walk sections in reverse to find the current one
        let found = '';
        ...
        // Only update state if it actually changed (avoids re-render)
        if (found !== prevSection.current) {
          prevSection.current = found;
  ...
  return (
    <div className="min-h-[100svh] antialiased bg-white overflow-x-hidden">
        ...
        <div className="relative z-10 w-full px-4 sm:px-6 lg:px-10 pb-20 xs:pb-24 sm:pb-28">
          ...
          >
            ...
            <p className="text-[13px] xs:text-sm sm:text-base lg:text-lg text-white/50 leading-relaxed max-w-[280px] xs:max-w-sm sm:max-w-md lg:max-w-lg mb-6 xs:mb-8 sm:mb-10">
              A hands‑on team that personally connects you with trusted truck operators for bulk and
              industrial cargo.
            </p>
      ...
      {/* ───────────── About ───────────── */}
      <section className="content-auto py-14 xs:py-16 sm:py-20 md:py-28 lg:py-36 scroll-mt-16" id="about">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-10">
          <motion.div {...FADE_UP}>
            ...
            <p className="text-[13px] xs:text-sm sm:text-[15px] text-slate-500 leading-[1.85] max-w-2xl mb-8 xs:mb-10 sm:mb-16">
              We're a family‑run logistics company based in Vasai since 2008. No call centres, no
              bots — when you ring us, you speak to someone who knows your shipment. We personally
              vet every truck operator and stay hands‑on from pickup to delivery.
            </p>
            ...
            <div className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
              {ABOUT_FEATURES.map((item, idx) => (
                <motion.div
                  ...
                  {...stagger(idx * 0.06)}
                >
                  <item.icon className="text-primary mb-2.5 sm:mb-3" size={20} strokeWidth={1.5} />
      ...
      {/* ───────────── Services ───────────── */}
      <section
        className="content-auto py-14 xs:py-16 sm:py-20 md:py-28 lg:py-36 bg-slate-50/70 scroll-mt-16"
        id="services"
      >
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-10">
          <motion.div className="max-w-xl mb-8 xs:mb-10 sm:mb-16" {...FADE_UP}>
            <p className="text-[10px] sm:text-[11px] font-semibold tracking-[0.2em] uppercase text-primary/60 mb-3 sm:mb-4">
              Services
            </p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 lg:gap-5">
            {SERVICES_DATA.map((service, idx) => (
              <motion.div
                ...
                {...stagger(idx * 0.06)}
              >
                <service.icon size={22} strokeWidth={1.5} className="text-primary mb-3 sm:mb-5" />
      ...
      {/* ───────────── Process ───────────── */}
      <section className="content-auto py-14 xs:py-16 sm:py-20 md:py-28 lg:py-36 scroll-mt-16" id="process">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-10">
          <motion.div className="text-center max-w-lg mx-auto mb-8 xs:mb-10 sm:mb-16" {...FADE_UP}>
            <p className="text-[10px] sm:text-[11px] font-semibold tracking-[0.2em] uppercase text-primary/60 mb-3 sm:mb-4">
              Process
            </p>
            <h2 className="text-2xl xs:text-[1.7rem] sm:text-3xl lg:text-4xl font-headline font-bold text-slate-900 leading-tight">
          ...
          <div className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-4 gap-3 xs:gap-4 sm:gap-5 lg:gap-6">
            {PROCESS_STEPS.map((item, idx) => (
              ...
              >
                ...
                <h3 className="text-sm font-semibold text-slate-900 mb-1 sm:mb-1.5">{item.title}</h3>
                <p className="text-[13px] sm:text-sm text-slate-400 leading-relaxed">{item.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 sm:gap-12 lg:gap-24">
            ...
            <motion.div {...FADE_UP}>
              ...
              <div className="flex items-center gap-3 sm:gap-4 mb-8 sm:mb-10">
                ...
                <div>
                  <h4 className="font-semibold text-slate-900 text-sm">Manoj Vyas</h4>
                  <p className="text-xs text-slate-400">Founder & Operations Head</p>
                </div>
              </div>
              ...
              <div className="space-y-3 sm:space-y-4 text-[13px] sm:text-sm text-slate-500">
                ...
                <a
                  ...
                  className="flex items-start sm:items-center gap-3 hover:text-primary transition-colors"
                >
                  <Mail size={16} className="text-primary flex-shrink-0 mt-0.5 sm:mt-0" strokeWidth={1.5} />
            ...
            {/* Right */}
            <motion.div className="space-y-3 sm:space-y-4" {...FADE_UP}>
              <div className="grid grid-cols-1 xs:grid-cols-2 gap-3 sm:gap-4">
                ...
                >
                  <MessageSquare size={20} strokeWidth={1.5} className="mb-4 sm:mb-6" />
                  <p className="text-[11px] xs:text-xs text-white/60 mb-1">Message us on</p>
                  <p className="text-lg sm:text-xl font-headline font-bold mb-3 sm:mb-5">WhatsApp</p>
                  <span className="flex items-center gap-1.5 text-xs font-medium text-white/60 group-hover:text-white/90 transition-colors">
              ...
              <div className="flex items-center gap-3 px-4 sm:px-5 py-3 sm:py-3.5 rounded-xl border border-slate-100 text-[13px] sm:text-sm text-slate-500 bg-white">
                <Clock size={14} className="text-primary flex-shrink-0" strokeWidth={1.5} />
                <span>
            ...
            <div className="grid grid-cols-1 xs:grid-cols-2 md:grid-cols-12 gap-8 sm:gap-10 md:gap-12 pb-8 sm:pb-12 border-b border-white/[0.08]">
              <div className="xs:col-span-2 md:col-span-5">
                ...
                <p className="text-[13px] sm:text-sm text-white/30 max-w-xs leading-relaxed">
                  Owner‑operated B2B logistics, moving freight across India since 2008.
                </p>
              </div>
              <div className="md:col-span-3 md:col-start-7">
                ...
                <ul className="space-y-2 sm:space-y-2.5">
                  {NAV_ITEMS.map((item) => (
                    <li key={item.label}>
                      <a className="text-[13px] sm:text-sm text-white/40 hover:text-white transition-colors" href={item.href}>
                        {item.label}
                      </a>
                    </li>
                  ))}
              ...
              <div className="md:col-span-3">
                ...
                <ul className="space-y-2 sm:space-y-2.5">
                  <li>
                    <a href="tel:9892414174" className="flex items-center gap-2 text-[13px] sm:text-sm text-white/40 hover:text-white transition-colors">
                      <Phone size={13} /> 989 241 4174
                    </a>
                  </li>
                  <li>
                    <a href="https://wa.me/919892414174" className="flex items-center gap-2 text-[13px] sm:text-sm text-white/40 hover:text-white transition-colors">
